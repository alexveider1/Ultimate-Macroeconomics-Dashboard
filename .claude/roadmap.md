...
- **#4 Eurostat (EU NUTS regions)** — new `extractors/eurostat.py` (Eurostat REST / JSON-stat, **keyless**); new `_configs/eurostat_download_config.json`. New **EU Regions** page — the second page under the now-existing **"Regional Statistics"** nav group — with a NUTS-2/3 choropleth (GISCO GeoJSON boundaries); reuse the region-agnostic `build_region_ranking_bar` / `build_region_trend_lines` helpers.
