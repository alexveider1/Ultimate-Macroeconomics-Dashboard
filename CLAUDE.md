...
| Service              | Port | Role                                                                                                                  |
| -------------------- | ---- | --------------------------------------------------------------------------------------------------------------------- |
| `db`                 | 5432 | Postgres 18 — World Bank + Yahoo Finance + Binance crypto + FRED US-state tabular data                                |
| `vector_db`          | 6333 | Qdrant — news article embeddings                                                                                      |
| `downloader_general` | —    | One-shot: bootstraps LLM role, ingests WB + Yahoo + Binance + FRED + news, populates both DBs                         |
...
| `downloader_extra`   | 8003 | FastAPI — on-demand WB indicator / Yahoo ticker / Binance pair / FRED state-indicator ingestion (called by the agent) |
...
3. **Workers** (one of `WORKER_NAMES` in `graph.py`) — every worker also receives the last ~3 chat turns so follow-ups disambiguate:
   ...
   - `downloader_agent` — calls `downloader_extra` to ingest data on demand from one of four sources (WB indicator, Yahoo ticker, Binance pair, FRED state indicator). Triggered when `sql_agent` returns `last_worker_status = NEEDS_DOWNLOAD`. The worker extracts a `DownloadPlan` (`source` + the source's id) and POSTs the unified `/ingest` body. For WB the exact `indicator_id`/`db_id` come from `sql_agent`'s `database_indicators` lookup; **Yahoo/Binance/FRED have no master catalogue**, so the worker infers the ticker / full USDT pair symbol / representative single-state FRED series id (Apple→`AAPL`, Solana→`SOLUSDT`, state unemployment→`CAUR`) from the user request and `downloader_extra` validates it against the live API.
...
For incremental additions during a live stack, the agent's `downloader_agent` worker calls `downloader_extra` (port 8003), which writes directly into the running Postgres without touching the marker. `downloader_extra` is **multi-source**: its single `POST /ingest` dispatches on a `source` field (`worldbank` / `yahoo` / `binance` / `fred`) to per-source clients (`client_wb.py`, `client_yahoo.py` using `yfinance`, `client_binance.py` + a trimmed `binance_client.py` copy, `client_fred.py` + a trimmed `fred_client.py` copy). Each path short-circuits when the item is already present (`status="already_downloaded"`), else delete-then-inserts the freshly fetched rows (metadata row first to satisfy the FK). World Bank has the `database_indicators` master catalogue so `sql_agent` finds the exact id before downloading; Yahoo/Binance/FRED have **no master table**, so the agent infers the ticker / full pair symbol / representative single-state FRED series id from world knowledge and `downloader_extra` validates it live (yfinance history for Yahoo, `/exchangeInfo?symbol=` for Binance, `geofred/series/group` for FRED) — an on-demand FRED item is stored under the upper-cased series id as its `indicator_id` slug with `category="On-demand (FRED)"`.
