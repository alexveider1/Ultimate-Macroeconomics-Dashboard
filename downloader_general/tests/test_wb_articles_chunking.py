"""Tests for the World Bank articles downloader: chunking, dedup, payload, clean."""
...
def test_build_chunk_entry_payload_shape() -> None:
    doc = {
        "id": "D9",
    ...
    entry = build_chunk_entry(doc, "chunk body", 0, 3, "inflation", "world_bank_inflation")
    ...
    assert article["title"] == "Inflation Report (part 1/3)"
    assert article["text"] == "chunk body"
    ...
    assert article["chunk_index"] == 0
    assert article["total_chunks"] == 3

    # A single-chunk document omits the "(part i/N)" suffix.
    single = build_chunk_entry(doc, "body", 0, 1, "inflation", "world_bank_inflation")
    assert single["article"]["title"] == "Inflation Report"
