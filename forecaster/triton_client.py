...
The forecasting models live in the ``triton`` container as python-backend models
that speak a single ``TYPE_STRING`` in / out contract (see ``triton/common``).
This adapter keeps its former HTTP contract and just forwards the heavy compute
to Triton. Kept as a tiny per-service module (duplicated in ``clustering``) in
line with the repo's no-shared-package convention.
"""
...
def infer_json(
    ...
    Args:
        ...
        model_name: Triton model to target (e.g. ``forecast_arima``).
